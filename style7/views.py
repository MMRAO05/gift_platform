...
class CreatePageView(View):
    """Serves the original 7.html page unchanged (setup + reveal
    in one template, exactly like the source file)."""

    def get(self, request):
        return render(request, "style7/page.html")
...
class RevealSeedView(View):
    """/style7/g/<short_code>/  — looks the gift up, then serves a tiny page
    that re-creates the EXACT url format (localstorage) the original page
    ...
    def get(self, request, short_code):
        ...
        gift.register_view()
        return render(request, "style7/seed_redirect.html", {
