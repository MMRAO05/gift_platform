...
class CreatePageView(View):
    """Serves the original 4.html page unchanged (setup + reveal
    in one template, exactly like the source file)."""

    def get(self, request):
        return render(request, "style4/page.html")
...
class RevealSeedView(View):
    """/style4/g/<short_code>/  — looks the gift up, then serves a tiny page
    that re-creates the EXACT url format (hash) the original page
    ...
    def get(self, request, short_code):
        ...
        gift.register_view()
        return render(request, "style4/seed_redirect.html", {
